...
def test_week2_end_to_end_integration(mock_delay, db, client, auth_header, seed_user):
    ...
    r_audit = client.get(f"/api/v1/audit?org_id={str(org.id)}&action=task.marked_overdue", headers=auth_header)
    ...
    client.patch(f"/api/v1/tasks/{escalation_task_id}/status", json={"status": "done"}, headers=auth_header)
    client.patch(f"/api/v1/tasks/{first_task['id']}/status", json={"status": "done"}, headers=auth_header)
