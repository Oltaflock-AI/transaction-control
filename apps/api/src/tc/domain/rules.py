...
def build_dedupe_key(rule_name: str, trigger: str, source_task_id: uuid.UUID) -> str:
    """
    Builds a deterministic dedupe key for a rule, trigger, and source task.
    
    Parameters:
        rule_name (str): Identifier of the rule.
        trigger (str): Trigger string that activated the rule.
        source_task_id (uuid.UUID): UUID of the source task.
    
    Returns:
        dedupe_key (str): Dedupe key in the format 'rule:{rule_name}:{trigger}:{source_task_id}'.
    """
...
def _is_dedupe_key_violation(exc: IntegrityError) -> bool:
    """
    Determine whether an IntegrityError represents a PostgreSQL unique constraint violation on the task `dedupe_key`.
    
    Returns:
        `True` if the error is a PostgreSQL error code 23505 and the violated constraint name contains "dedupe_key", `False` otherwise.
    """
...
def _resolve_coordinator(db: Session, transaction_id: uuid.UUID) -> uuid.UUID | None:
    """
    Resolve the coordinator for a transaction by selecting the first admin member of the transaction's organization.
    
    Returns:
        uuid.UUID | None: The user ID of the selected admin coordinator (chosen by ascending user_id order), or `None` if the transaction does not exist or the organization has no admin members.
    """
...
def evaluate_rules(
    ...
    *,
    ...
    """
    Evaluate all rules that match the given trigger and create any corresponding follow-up tasks.
    
    Parameters:
        trigger (str): The trigger identifier to match against rule definitions.
        source_task (Task): The task whose context is used to build follow-up tasks (title, transaction, id).
    
    Returns:
        list[Task]: Newly created Task objects; empty if no tasks were created due to deduplication.
    """
