...
def check_deadlines(db: Session) -> dict:
    """
    Check tasks' deadlines and record overdue and due-soon events.
    
    Marks tasks whose due time is past as overdue, creates corresponding EventLog and audit entries, and invokes rule evaluation for each marked task. Detects tasks due within the next 48 hours, creates a single "due soon" EventLog and audit entry per task when not already logged, invokes rule evaluation, and commits the database session.
    
    Returns:
        result (dict): Summary of the operation containing:
            - "checked_at": ISO 8601 timestamp when the check ran.
            - "overdue_marked": number of tasks updated to overdue.
            - "due_soon_logged": number of newly created due-soon event entries.
    """
