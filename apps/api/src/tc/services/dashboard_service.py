...
def get_agent_dashboard(db: Session, user_id: uuid.UUID, org_id: uuid.UUID):
    ...
    total_txns = (
        ...
        .join(Transaction)
        .filter(Task.assignee_id == user_id, Transaction.org_id == org_id, Transaction.status != "closed")
        .scalar() or 0
    )
    ...
    tasks_overdue = (
        ...
        .join(Transaction)
        .filter(Task.assignee_id == user_id, Transaction.org_id == org_id, Task.status == TaskStatus.overdue)
        .scalar() or 0
    )
